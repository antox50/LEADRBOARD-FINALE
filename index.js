...
client.on('message', message => {
    if (message.content === '!showdb') {
        db.all("SELECT username, points FROM users", (err, rows) => {
            ...
            rows.forEach(row => {
                nicknames += row.username + '\n'; // Aggiungi una riga di spazio tra ogni nickname
                points += row.points + '\n';
            });

            embed.addField('Nickname', nicknames, true); // Aggiungi spazio per allontanare i campi
            embed.addField('\u200B', '\u200B', true); // Aggiungi un campo vuoto per separare
            embed.addField('Tasks', points, true);
